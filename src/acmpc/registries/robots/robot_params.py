...
class RobotParams(BaseModel):
    ...
    max_angular_acc: float = Field(
        gt=0,
        description="Maximum angular acceleration (rad/s²)"
    )
    ...
    wheel_base: float = Field(
        gt=0,
        description="Wheel base length (m)"
    )
    lidar_range: float = Field(
        gt=0,
        description="LiDAR maximum range (m)"
    )
    lidar_rays: int = Field(
        gt=0,
        description="Number of LiDAR rays"
    )
    lidar_fov: float = Field(
        gt=0,
        description="LiDAR field of view (rad)"
    )